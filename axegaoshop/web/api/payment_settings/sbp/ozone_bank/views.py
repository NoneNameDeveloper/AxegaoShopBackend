...
from axegaoshop.web.api.payment_settings.sbp.ozone_bank.schema import PaymentSettingsCreate, PaymentSettingsIn
...
async def create_payment_setting_ozone_bank(payment_settings: PaymentSettingsCreate):
    ...
    ozone_bank = await get_ozone_bank_raw(payment_settings.token, payment_settings.pin_code)

    if not ozone_bank:
        raise HTTPException(status_code=401, detail="INVALID_DATA")

    await PaymentSettingsOzone().create(**payment_settings.model_dump(exclude_unset=True))
