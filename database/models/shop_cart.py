...
from database.models.product import Product, Parameter
...
class ShopCart(Model):
    ...
    user: fields.ForeignKeyRelation = fields.ForeignKeyField(
        model_name="axegaoshop.User",
        related_name="shop_cart"
    )
    product = fields.ForeignKeyField(
        "axegaoshop.Product", related_name="shop_cart"
    )
    parameter = fields.ForeignKeyField(
        "axegaoshop.Parameter", related_name="shop_cart"
    )
...
async def add_to_cart(user_id: int, product_id: int, parameter_id: int, quantity: int) -> ShopCart:
    ...
    _cart = await ShopCart.filter(product=product, user=user, parameter=parameter).first()
