...
from schemas.tokens import TokenCreated, TokenRequest
from schemas.users import UserCreate, UserOutput, UserUpdate, UserIn_Pydantic, UserCart_Pydantic, UserForAdmin_Pydantic, \
    UserUpdateAdmin

from security.auth_bearer import JWTBearer
from security.security import get_hashed_password, verify_password, create_refresh_token, create_access_token

from utils.images import create_user_photo
from utils.users_misc import get_current_user, current_user_is_admin
...
async def login_user(request: TokenRequest):
    ...
    if not request.password:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Provide password")
