...
from database.models.subcategory import Subcategory, change_subcategory_order

from schemas.subcategories import SubcategoryIn_Pydantic, SubcategoryCreate, SubcategoryUpdate, SubcategoryOrderChange
...
async def subcategory_create(subcategory: SubcategoryCreate):
    ...
    return await SubcategoryIn_Pydantic.from_queryset_single(Subcategory.filter(id=subcat.id).first())
...
async def category_subcategory_get(category_id: int):
    if not await Category.get_or_none(id=category_id):
        raise HTTPException(status_code=404, detail="CATEGORY_NOT_FOUND")
    return await SubcategoryIn_Pydantic.from_queryset(Subcategory.filter(category_id=category_id).all())
...
async def subcategory_update(id: int, subcategory_: SubcategoryUpdate):
    ...
    await Subcategory.filter(id=id).update(**subcategory_.model_dump(exclude_unset=True))

    return await SubcategoryIn_Pydantic.from_queryset_single(Subcategory.filter(id=id).first())
